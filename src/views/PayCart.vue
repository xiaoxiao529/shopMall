					...
					<div v-else>
						...
						<div class="cart-table">
							<div class="cart-group">
								<!--购物列表-->
								<div class="cart-top-items" :key="index" v-for="(item,index) in shopCarData">
									<div class="cart-items">
										<div class="items-choose">
											<span class="blue-checkbox-new" @click="changeGoodCheckHandle(item)" :class="{'checkbox-on':item.checked}"><a></a></span>
										</div>
										...
										<div class="operation">
											<a class="items-delete-btn" @click="deleteGood(item)"></a>
										</div>
										<div class="subtotal">¥ {{item.price*item.count}}</div>
										<div class="item-cols-num">
											<div class="select js-select-quantity">
												<span class="down" :class="{'down-disabled':item.count===1}" @click="reduceCartNumHandle(item)">-</span>
												<span class="num">
													<input type="text" :value="item.count" style="display: inline-block;">
													<ul>
														<li>1</li>
														<li>2</li>
													</ul>
												</span>
												<span class="up" :class="{'up-disabled':item.count===item.limit_num}" @click="addCartNumHandle(item)">+</span>
					...
					<div class="cart-bar-operation">
						<div>
							<div class="choose-all js-choose-all">
								<span class="blue-checkbox-new" :class="{'checkbox-on':totalChecked}" @click="checkAllGoodsHandle(totalChecked)"><a></a></span>
								全选
							</div>
							<div class="delete-choose-goods" @click="deleteCheckedGoodHandle()">删除选中的商品</div>
						</div>
					</div>
					<div class="shipping">
						<div class="shipping-box">
							...
							<div class="shipping-total shipping-price">
								<h4 class="">
									应付总额：<span>￥</span><i >{{totalCheckedPrice}}</i> 
								</h4>
						...
						<router-link tag="span" class="jianguo-blue-main-btn big-main-btn js-checkout" :class="{'disabled-btn':totalChecked<1}" to="/checkout">
							<a>现在结算</a>
						</router-link>
...
export default{
	...
	computed:{
		shopCarData(){  //这里拿到的也是购物车数据，存在Vuex里面，全局通用，过滤、操作的都是购物车数据
		...
		totalCheckedCount(){
			return this.$store.getters.totalCheckedCount
		},
		totalCheckedPrice(){
			return this.$store.getters.totalCheckedPrice
		}
	},
	methods:{
		addCartNumHandle(item){  //通过mutations，加就在目前的基础上加就行了，不需要新开一个变量存储，然后在赋值到现在的这个商品上，直接获取到现在的count数量，然后直接++
			this.$store.commit('addCartNum',item);
		},
		reduceCartNumHandle(item){  
			this.$store.commit('reduceCartNum',item);
		},
		deleteGood(item){
			/*let index = this.shopCarData.indexOf(obj)  //不可以在组件里面直接操作Vuex里面的数据，需要通过commit触发mutations事件
			this.shopCarData.splice(index,1)*/
			this.$store.commit('deleteFromCar',item)
		},
		changeGoodCheckHandle(item){
			this.$store.commit('changeGoodCheck',item)
		},
		checkAllGoodsHandle(totalChecked){
			this.$store.commit('checkAllGoods',totalChecked)
		},
		deleteCheckedGoodHandle(){
			this.$store.commit('deleteCheckedGood')
		}
	}
}
</script>
