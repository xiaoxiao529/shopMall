			...
			<div class="gray-box">
						...
						<div class="cart-group js-cart-group">
							<div class="cart-items" v-for="(item,index) in checkedGoods">
								...
								<div class="name hide-row">
									<div class="name-cell"> 
										<a href="javascript:;" :title="item.sub_title+'（'+item.spec_json.show_name+'）'" target="_blank">
											{{item.sub_title+'（'+item.spec_json.show_name+'）'}}
										</a>
									</div>
								</div>
								<div class="subtotal">
				...
				<div class="box-inner"> 
					<div class="order-discount-line"> 
						<p> 商品总计： <span>¥ {{totalCheckedPrice}}.00</span> </p> 
...
export default{
	...
	computed:{  //这里只能拿选中的商品数据过来，不是把购物车商品全部拿过来
		...
		totalCheckedPrice(){  //总价也是选中的商品的价格，而不是购物车里商品的总价格
			return this.$store.getters.totalCheckedPrice
		},
		freight(){
			let freight = 8;
			if(this.totalCheckedPrice>88){
				freight = 0;
			}
			return freight;
		},
