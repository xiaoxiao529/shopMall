                            ...
                            <div class="address-form">
                                <div class="module-form-row">
                                    <div class="form-item-v3">
                                        ...
                                        <input placeholder="收货人姓名" type="text" v-model="addressList.name" class="js-verify">
                                        <div class="verify-error"></div>
                                    </div>
                                </div>
                                <div class="module-form-row">
                                    <div class="form-item-v3" :class="{'form-invalid-item':phoneCheck}">
                                        <input placeholder="手机号" type="text" v-model="addressList.phone" class="js-verify">
                                        <div class="verify-error" v-show="phoneCheck">手机号码不正确</div>
                                    </div>
                                </div>
                                ...
                                <div class="module-form-row clear">
                                    <div class="form-item-v3 select-item province-wrapper">
                                        <select name="province_code" class="province select-province js-form-province js-verify" v-model="addressList.provinceId">
                                            <option value="0">请选择省份</option> 
                                            <option :value="item.area_id"  v-for="item,index in ProvinceList">{{item.area_name}}</option>
                                            <!--value="0"这个为0的value值，就是v-model绑定的addressList.provinceId值，只要选择"请选择省份"，value值就为0-->
                                            <!--设置ddressList.provinceId值为0，那么相对应的value值就是0，双向数据绑定-->
                                        </select>
                                    </div>
                                </div>
                                <div class="module-form-row clear">
                                    <div class="form-item-v3 select-item city-wrapper fn-left form-focus-item" >
                                        <select class="city select-city js-form-city js-verify" v-model="addressList.cityId">
                                        	<option value="0">请选择城市</option>
                                            <option :value="item.area_id"  v-for="(item,index) in cityList">{{item.area_name}}</option>
                                        </select>
                                    </div>
                                    <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item" >
                                        <select class="city select-city js-form-city js-verify" v-model="addressList.countyId">
                                        	<option value="0">请选择区县</option>
                                            <option :value="item.area_id" v-for="(item,index) in countyList">{{item.area_name}}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="module-form-row">
                                    <div class="form-item-v3">
                                        <input v-model="addressList.add" placeholder="详细地址，如街道名称，楼层，门牌号码等" type="text" class="js-verify">
                                        <div class="verify-error"></div>
                                    </div>
                                </div>
                                <div class="module-form-row fn-clear">
                                    <input type="checkbox" class="hide">
                                    <span class="blue-checkbox " :class="{'blue-checkbox-on':defaultCheck}" @click="checkDefault()"></span>设为默认
                                </div>
                                <div class="dialog-blue-btn big-main-btn js-verify-address" :class="{'disabled-btn':!infoRightCheck}">
                                    <a @click="commitInfo()">保存</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default{
		...
		return {
			...
			addressList:{
				"name": "",
	          	...
	          	"areaCode": "",
	          	"landLine": "",
	          	"provinceId": 0,  //v-model="addressList.provinceId" 下拉框选什么值这里就改成了什么值    :value="item.area_id"这个是从数据里取的值绑定到value上
	          	"province": "",
	          	"cityId": 0,
	          	"city": "",
	          	"countyId": 0,
			...
			phoneCheck:false,
			defaultCheck:false,
			infoRightCheck:false
		}
	},
	watch:{
		'addressList.countyId'(){  //addressList.countyId的值会随着下拉框值的改变而改变
			this.countyList.forEach((item)=>{  //这里面遍历countyList的值
				if(item.area_id == this.addressList.countyId){
					this.addressList.county = item.area_name
					return
				}
			})
		},
		'addressList.phone'(){
			if(this.addressList.phone.length!=11){
	...
	computed:{
		cityList(){
			...
			this.ProvinceList.forEach((item,index)=>{
				if(item.area_id == this.addressList.provinceId){  //先获取对应的省份，通过id获取，然后获取下面的第一级的市县，赋值给变量市县
					this.addressList.province = item.area_name  //这个用于把信息存到vuex里面的addressData
					cityList = item.city_list
					return
				}
			})
			if(this.addressList.provinceId == '0'){
				this.addressList.cityId = 0
			}
			return cityList			
		},
		countyList(){  //上面的cityList，这里面也可以拿到，因为是计算属性
			let countyList = []
			this.cityList.forEach((item,index)=>{
				if(item.area_id==this.addressList.cityId){
					this.addressList.city = item.area_name
					countyList = item.county_list
				}
			})
			if(this.addressList.cityId == '0'){
				this.addressList.countyId = 0
			}
			return countyList
		}
	},
	methods:{
		...
		checkDefault(){
			...
			this.addressList.default = !this.addressList.default
		},
		checkInfoRight(){
			console.log(!this.phoneCheck)
			if( this.addressList.name && this.addressList.phone && !this.phoneCheck && this.addressList.provinceId && this.addressList.cityId && this.addressList.countyId & this.addressList.add ){
				this.infoRightCheck = true
			}else{
				
				this.infoRightCheck = false
			}
		},
		commitInfo(){
			if(this.infoRightCheck){
				this.$store.commit('addCustomInfo',this.addressList)
				this.$emit('closePop')
			}
		}
	}
...
#pop  .blue-checkbox-on{
	background: url(../assets/img/checkbox-bg.png) no-repeat 0 -20px;
}
#pop .dialog-blue-btn{
    padding: 1px;
    background: #6383C6;
