    	...
    	<div class="content page-order-payment">
    	    <div class="gray-box clear">
    	        ...
    	        <div class="box-inner payment-checkout-panel clear">
    	            ...
    	            <span class="prices"> 应付金额：   <em><span>¥ </span>{{orderData.price}}.00</em>   </span>
    	        </div>
    	    </div>
    	    <div class="confirm-detail">
    	        <div class="info-title">订单编号</div>
    	        <p class="info-detail">{{$route.query.orderId}}</p>
    	    </div>
    	    <div class="confirm-detail">
    	        <div class="info-title">收货信息</div>
    	        <p class="info-detail">姓名：{{orderData.customAddress.name}}</p>
    	        <p class="info-detail">联系电话：{{orderData.customAddress.phone}}</p>  
    	        <p class="info-detail">详细地址：{{orderData.customAddress.province}}{{orderData.customAddress.city}}{{orderData.customAddress.county}}</p>
    	    </div>
    	    <div class="confirm-detail">
    	        <div class="info-title">发票信息</div>
    	        <p class="info-detail">发票类型：电子发票</p>
    	        <p class="info-detail">发票抬头：{{orderData.invoiceTitle}}</p>
    	    ...
    	    <div class="order-discount-line">
    	        <p> 商品总计： <span>¥ {{orderData.price}}.00</span> </p>
...
export default{
	computed:{
		orderData(){
			return this.$store.state.orderData.filter((item)=>{
				return item.orderId == this.$route.query.orderId
			})[0]  //this.$store.state.orderData是一个数组，要取出的是对象，这个数组只有一个元素
			/*{  //数据格式
				orderId:new Date().getTime(), 
				goodsData:this.checkedGoods,  
				invoiceTitle:invoiceTitle,  
				price:this.totalCheckedPrice,  
